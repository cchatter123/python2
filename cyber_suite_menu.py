# FILE NAME: cyber_suite_menu.py

# NAME: Oliver Doty
# DATE: 2/10/2025
# BRIEF DESCRIPTION:  Create a code that outputs a menu for the cyber suite



# 1. Make sure you fill out the comments above
# 2. Write your code in the proper spot
# 3. Be sure to answer the Reflection Questions below
# 4. The Sample Output has been included in this code for your convenience



########## ENTER YER CODE BELOW THIS LINE ##########

print("WELCOME TO THE cyber_suite")
print()
print("Please choose from the following menu:")
print("  1. Password manager")
print("  2. Examine logs")
print("  3. Scraper website")
print("  4. Hash")
print("  5. Encrypt")
print("  6. Encode")
print("  7. Steganography")
print("  8. EXIT")
print()
print("Enter selection:")









########### END YER CODE ABOVE THIS LINE ###########
    


########################################
#          SAMPLE OUTPUT
########################################

'''
WELCOME TO THE cyber_suite

Please choose from the following menu:
  1. Password manager
  2. Examine logs
  3. Scrape website
  4. Hash
  5. Encrypt
  6. Encode
  7. Steganography
  8. EXIT

Enter selection:
'''



########################################
#          REFLECTION QUESTIONS
########################################

'''
1. This is the last exercise for the chapter! Imagine you are talking to a new Python learner. What
   advice do you have for someone who is just getting started and getting ready to do their first lab?

Don't stress it. It looks like a lot but it really isnt that bad. Baby steps



'''